...
const flipCard = (index: number) => {
  const visibleCards = gameState.value.cards.filter(
    (card) => !card.flipped && card.active,
  );
...
const checkForMatch = () => {
  ...
  const visibleCards = newGameState.cards.filter(
    (card) => !card.flipped && card.active,
  );
  if (visibleCards.length == 2) {
    ...
    if (c1.id == c2.id) {
      ...
      if (newGameState.firstCard) {
        ...
        const otherCardKey =
          newGameState.firstCard.id +
          "@" +
          (newGameState.firstCard.pairIndex == 0 ? 1 : 0);
  ...
  newGameState.runState =
    activeCards.length == 0 ? RunState.STOPPED : RunState.STARTED;
  newGameState.firstCard =
    visibleCards.length == 1 ? visibleCards[0] : undefined; // remember the first card for bad misses
  gameState.value = newGameState;
};
